---
title: "Reproducing Figure 6"
author: "Kevin Jin"
date: "2024-06-06"
output: html_document
---

```{r, regression_code/set_up_regression.R}
#source("regression_code/regression.R") in the next code chunk, modified by removing print statement
...
```{r, regression_code/regression.R}
###################################################################################################
...
```{r}
load("results/regression_linear_influential.Rdata")
```

```{r}
for(i in 1:10){
  print(result[["2"]][[i]]$selected$masking)
  print(result[["2"]][[i]]$projected$masking)
}
```
```{r, eval = F}
##
...
```{r}
find_y_lims <- function(obs){
  y_max = -1000
  y_min = 1000
  select_num <- length(obs$selected$split)
  for(i in 1:select_num){
    new_y_min = obs$projected$split[i] +obs$CIs$split[i,1]
    new_y_max = obs$projected$split[i] +obs$CIs$split[i,2]
    if(new_y_min < y_min){
      y_min = new_y_min
    }
    if(new_y_max > y_max){
  ...
  select_num <- length(obs$selected$masking)
  for(i in 1:select_num){
    new_y_min = obs$projected$masking[i] +obs$CIs$masking[i,1]
    new_y_max = obs$projected$masking[i] +obs$CIs$masking[i,2]
    if(new_y_min < y_min){
      y_min = new_y_min
    }
    if(new_y_max > y_max){
  ...
  select_num <- length(obs$selected$full)
  for(i in 1:select_num){
    new_y_min = obs$projected$full[i] +obs$CIs$full[i,1]
    new_y_max = obs$projected$full[i] +obs$CIs$full[i,2]
    if(new_y_min < y_min){
      y_min = new_y_min
    }
    if(new_y_max > y_max){
  ...
  return(list(y_minimum = y_min,y_maximum = y_max))
  
}

instance_plot <- function(obs){
  y_lims = find_y_lims(obs = obs)
  y_min = y_lims$y_minimum
  y_max = y_lims$y_maximum
  temp_df <- data.frame(obs$CIs$"masking") |>
    rename(lb = X1, ub = X2)
  temp_df$true = as.list(obs$projected$masking)
  temp_df$selected = as.list(obs$selected$masking)
  temp_df <- temp_df |>
    mutate(correct = as.factor(ifelse(lb <= true & true <= ub, 1, 0)),
           selected = as.numeric(selected),
           true = as.numeric(true))
  g1 <- make_CI_plot(temp_df, "Data Fission", y_min, y_max)
  g1
  temp_df <- data.frame(obs$CIs$full) |>
    rename(lb = X1, ub = X2)
  temp_df$true = as.list(obs$projected$full)
  temp_df$selected = as.list(obs$selected$full)
  temp_df <- temp_df |>
    mutate(correct = as.factor(ifelse(lb <= true & true <= ub, 1, 0)),
           selected = as.numeric(selected),
           true = as.numeric(true))
  g2 <- make_CI_plot(temp_df, "Full", y_min, y_max)
  temp_df <- data.frame(obs$CIs$split) |>
    rename(lb = X1, ub = X2)
  temp_df$true = as.list(obs$projected$split)
  temp_df$selected = as.list(obs$selected$split)
  temp_df <- temp_df |>
    mutate(correct = as.factor(ifelse(lb <= true & true <= ub, 1, 0)),
           selected = as.numeric(selected),
           true = as.numeric(true))
  g3<- make_CI_plot(temp_df, "Data Splitting", y_min, y_max)
...
make_CI_plot <- function(df, type, y_min, y_max){
  g = ggplot(df) +
    geom_errorbar(aes(x = selected, ymin = lb, ymax = ub, color = correct)) + 
    ylim(y_min, y_max) +
    xlim(0, 20) + 
    scale_color_manual(values=c("red", "black"), breaks = c(0,1)) + 
    geom_point(x = 0, y = 0, size = 0.5) +
    geom_point(x = 1, y = 1, size = 0.5) + 
    geom_segment(x = 2, y = 0, xend = 16, yend = 0, linetype = "dotted", size = 1.08) + 
    geom_point(x = 17, y = 1, size = 0.5) + 
    geom_point(x = 18, y = -1, size = 0.5) + 
    geom_point(x = 19, y = 1, size = 0.5) + 
    geom_point(x = 20, y = 0, size = 0.5) +
    labs(title = type, y= "coeffecient") + 
    theme_bw() +
...
library(gridExtra)
obs <- result[["2"]][[7]]
g <- instance_plot(obs); g
ggsave("figures/Leverage = 2.png", plot = g, width = 12, units = "in")

obs <- result[["6"]][[500]]
g <- instance_plot(obs); g
ggsave("figures/Leverage = 6.png", plot = g, width = 12, units = "in")
